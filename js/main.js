...
btnMultMx.addEventListener("click", function (e) {
    ...
    var a = mxObjA.getArray(), // достанем массивы из объектов
        b = mxObjB.getArray();

    mxObjC.setArray( mulMatrix(a, b) ); // перемножим их и присвоим объекту матрицы С
});
...
btnExchangeMx.addEventListener("click", function (e) {
    var temp = mxObjA.getArray();
    mxObjA.setArray(mxObjB.getArray());
    mxObjB.setArray(temp);

    mxObjC.fillWithEmptyCells(mxObjB.getWidth(), mxObjA.getHeight());
    checkSizes();
});
...
function checkSizes() {
    if (mxObjA.getWidth() != mxObjB.getHeight()){
...
function checkMatrix() {
    var message = '';
    if (!isMatrix(mxObjA.getArray())){
        message += mxObjA.name.toUpperCase() + ': ' + MESSAGES.WRONG_MATRIX;
    }
    if (!isMatrix(mxObjB.getArray())){
